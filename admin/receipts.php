...
if (isset($_GET['mark_paid'])) {
    ...
    $stmt = $db->prepare("UPDATE receipts SET payment_status = 'paid', paid_at = NOW() WHERE id = ?");
    $stmt->execute([$receipt_id]);
    
    header("Location: receipts.php?" . $_SERVER['QUERY_STRING']);
    exit();
}
...
$stats_query = "
    SELECT 
        ...
        SUM(CASE WHEN r.payment_status = 'paid' THEN r.final_amount ELSE 0 END) as paid_amount,
        SUM(CASE WHEN r.payment_status = 'pending' THEN 1 ELSE 0 END) as pending_count
    FROM receipts r
    JOIN appointments a ON r.appointment_id = a.id
    WHERE r.issued_at BETWEEN :start_date AND :end_date + INTERVAL 1 DAY
";
    ...
    <div class="admin-container">
        ...
        <div class="admin-content">
            ...
            <div class="search-form">
                <form method="GET">
                            ...
                            <select id="payment_status" name="payment_status">
                                ...
                                <?php foreach ($payment_statuses as $value => $label): ?>
                                    <option value="<?php echo $value; ?>" 
                                            <?php echo ($payment_status == $value) ? 'selected' : ''; ?>>
                                        <?php echo $label; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="filter-actions">
                        ...
                        <button type="button" onclick="printReceipts()" class="print-btn">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-number"><?php echo number_format($stats['total_amount'] ?? 0, 0, ',', ' '); ?> ₽</div>
                ...
                <div class="stat-card">
                    <div class="stat-number"><?php echo number_format($stats['paid_amount'] ?? 0, 0, ',', ' '); ?> ₽</div>
                    <div class="stat-label">Оплаченная сумма</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number"><?php echo number_format($stats['avg_amount'] ?? 0, 0, ',', ' '); ?> ₽</div>
                ...
                <table>
                    ...
                    <tbody>
                        <?php if (empty($receipts)): ?>
                            <tr>
                                <td colspan="9" style="text-align: center;">Чеки не найдены</td>
                            </tr>
                        <?php else: ?>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <div class="receipt-details">
                                        <small>
                                            Запись #<?php echo $receipt['appointment_id']; ?><br>
                                            <?php echo date('d.m.Y', strtotime($receipt['appointment_date'])); ?> 
                                            <?php echo date('H:i', strtotime($receipt['start_time'])); ?>
                                        </small>
                                    </div>
                                </td>
                                <td>
                                    <?php echo date('d.m.Y H:i', strtotime($receipt['issued_at'])); ?>
                                </td>
                                <td>
                                    <strong><?php echo htmlspecialchars($receipt['client_name']); ?></strong>
                                    <div style="font-size: 12px; color: #666;">
                                        <?php echo htmlspecialchars($receipt['client_phone']); ?>
                                    </div>
                                </td>
                                <td><?php echo htmlspecialchars($receipt['master_name']); ?></td>
                                <td>
                                    <?php echo htmlspecialchars($receipt['service_name']); ?>
                                    <div style="font-size: 12px; color: #666;">
                                        Цена: <?php echo number_format($receipt['service_price'], 0, ',', ' '); ?> ₽
                                    </div>
                                </td>
                                <td>
                                    <div class="amount">
                                        <?php echo number_format($receipt['final_amount'], 0, ',', ' '); ?> ₽
                                    </div>
                                    <?php if ($receipt['discount'] > 0): ?>
                                        <div style="font-size: 12px; color: #e74c3c;">
                                            Скидка: <?php echo number_format($receipt['discount'], 0, ',', ' '); ?> ₽
                                        </div>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    ...
                                    ?>
                                    <span class="badge <?php echo $status_badges[$receipt['payment_status']] ?? 'badge-info'; ?>">
                                        <?php echo $status_labels[$receipt['payment_status']] ?? $receipt['payment_status']; ?>
                                    </span>
                                    <?php if ($receipt['payment_status'] == 'paid' && $receipt['paid_at']): ?>
                                        <div style="font-size: 12px; color: #666; margin-top: 5px;">
                                            Оплачен: <?php echo date('d.m.Y H:i', strtotime($receipt['paid_at'])); ?>
                                        </div>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    ...
                                    ?>
                                    <span class="<?php echo $method_classes[$receipt['payment_method']] ?? ''; ?>">
                                        <?php echo $method_labels[$receipt['payment_method']] ?? $receipt['payment_method']; ?>
                                    </span>
                                </td>
                                <td>
                                    <div class="action-buttons">
                                        <?php if ($receipt['payment_status'] == 'pending'): ?>
                                            <a href="?<?php echo http_build_query(array_merge($_GET, ['mark_paid' => $receipt['id']])); ?>" 
                                               class="btn btn-sm btn-success"
                                               onclick="return confirm('Отметить чек #<?php echo $receipt['id']; ?> как оплаченный?')"
                                        ...
                                        <?php if ($receipt['payment_status'] == 'paid'): ?>
                                            <a href="?<?php echo http_build_query(array_merge($_GET, ['mark_cancelled' => $receipt['id']])); ?>" 
                                               class="btn btn-sm btn-danger"
                                               onclick="return confirm('Отметить чек #<?php echo $receipt['id']; ?> как возврат?')"
                                        ...
                                        <button onclick="showReceiptDetails(<?php echo $receipt['id']; ?>)" 
            ...
            <div class="pagination">
                <?php if ($page > 1): ?>
                    <a href="?<?php echo http_build_query(array_merge($_GET, ['page' => $page - 1])); ?>">&laquo; Назад</a>
                <?php endif; ?>
                ...
                <?php if ($page < $totalPages): ?>
                    <a href="?<?php echo http_build_query(array_merge($_GET, ['page' => $page + 1])); ?>">Вперед &raquo;</a>
                <?php endif; ?>
            </div>
            <?php endif; ?>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                <strong>
                                    ...
                                    echo $method_labels[$stat['payment_method']] ?? $stat['payment_method'];
                                    ?>
                                </strong>
                            </div>
                            <div class="badge badge-info"><?php echo $stat['count']; ?> чеков</div>
                        </div>
                        <div style="margin-top: 10px; font-size: 18px; color: #27ae60; font-weight: bold;">
    ...
    <div id="receiptModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000;">
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 30px; border-radius: 10px; width: 90%; max-width: 600px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h2 style="margin: 0;">Детали чека</h2>
                <button onclick="closeReceiptModal()" style="background: none; border: none; font-size: 24px; cursor: pointer;">&times;</button>
            </div>
            <div id="receiptDetails"></div>
            <div style="text-align: center; margin-top: 20px;">
                <button onclick="closeReceiptModal()" class="btn btn-primary">Закрыть</button>
            </div>
        </div>
    </div>
    ...
    function showReceiptDetails(receiptId) {
        fetch('get_receipt_details.php?id=' + receiptId)
            ...
            .catch(error => {
                ...
                alert('Ошибка при загрузке деталей чека');
            });
    }
