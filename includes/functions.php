<?php
// functions.php - Общие функции для проекта (адаптировано под PostgreSQL)
...
function createAppointment($db, $data) {
    try {
        ...
        if (empty($data['client_id'])) {
            // В PostgreSQL email и password_hash обязательны. Если разрешить NULL, можно упростить.
            // Пока генерируем временные значения
...
function getStatistics($db, $period = 'today') {
    ...
    switch ($period) {
        case 'today':
            $date_condition = "appointment_date = CURRENT_DATE";
            break;
        case 'week':
            $date_condition = "appointment_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days'";
            break;
        case 'month':
            $date_condition = "EXTRACT(MONTH FROM appointment_date) = EXTRACT(MONTH FROM CURRENT_DATE) AND EXTRACT(YEAR FROM appointment_date) = EXTRACT(YEAR FROM CURRENT_DATE)";
            break;
        default:
            $date_condition = "TRUE";
    }
